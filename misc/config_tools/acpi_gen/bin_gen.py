...
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'static_allocators'))
import gpa
import lib.cdata
import common
import struct

def move_rtct_ssram_entries(rtct, new_base_addr, new_area_max_size):
    '''
    move the ssram entries to a new area. the entries keeps their relative layout
    ...
    sum = 0
    m=bytes(rtct)
    for i in range(0, rtct.header.length):
        sum += m[i]
    rtct.header.checksum = 0 - sum
    ...
    for acpi_table in ACPI_TABLE_LIST:
        if acpi_table[0] == 'tpm2.asl':
            ...
            if 'PTCT' in os.listdir(dest_vm_acpi_path):
                ...
                move_rtct_ssram_entries(rtct, gpa.PRE_RTVM_SW_SRAM_BASE_GPA, gpa.PRE_RTVM_SW_SRAM_MAX_SIZE)
