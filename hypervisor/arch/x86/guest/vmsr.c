...
int32_t wrmsr_vmexit_handler(struct acrn_vcpu *vcpu)
{
	...
	{
		if (is_vhwp_configured(vcpu->vm)) {
			msr_write(msr, (v & (~MSR_IA32_HWP_REQUEST_RSV_BITS)) & (~MSR_IA32_HWP_REQUEST_PC));
		} else {
			vcpu_inject_gp(vcpu, 0U);
		}
		break;
	...
	{
		if (is_vhwp_configured(vcpu->vm)) {
			msr_write(msr, v & (~MSR_IA32_HWP_STATUS_RSV_BITS));
		} else {
			vcpu_inject_gp(vcpu, 0U);
		}
		break;
...
void update_msr_bitmap_x2apic_passthru(struct acrn_vcpu *vcpu)
{
	...
	enable_msr_interception(msr_bitmap, MSR_IA32_EXT_APIC_LVT_THERMAL, INTERCEPT_WRITE);
