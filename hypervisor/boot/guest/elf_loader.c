 ...
 */
#define ELF_LOAD_PARAMS_SIZE			(MEM_1K * 27U)
#define ELF_INITGDT_GPA(load_params_gpa)	((load_params_gpa) + 0UL)
#define ELF_LDR_NAME_GPA(load_params_gpa)	((load_params_gpa) + 64UL)
#define ELF_RSDP_GPA(load_params_gpa)		((load_params_gpa) + 128UL)
#define ELF_MB_INFO_GPA(load_params_gpa)	((load_params_gpa) + MEM_1K)
#define ELF_CMDLINE_GPA(load_params_gpa)	((load_params_gpa) + MEM_1K + MEM_4K)
#define ELF_MMAP_GPA(load_params_gpa)		((load_params_gpa) + MEM_1K + MEM_4K + MEM_2K)

int32_t prepare_elf_cmdline(struct acrn_vm *vm, uint64_t param_cmd_gpa)
{
	return copy_to_gpa(vm, (void *)hva2hpa(vm->sw.bootargs_info.src_addr), param_cmd_gpa, vm->sw.bootargs_info.size);
}
...
uint32_t prepare_multiboot_mmap(struct acrn_vm *vm, uint64_t param_mmap_gpa)
{
	...
	for (i = 0U; i < vm->e820_entry_num; i++) {
		...
		if (copy_to_gpa(vm, (void *)hva2hpa(&mmap_entry), mmap_gpa,
...
int32_t elf_loader(struct acrn_vm *vm)
{
	 ...
	 */
	uint64_t load_params_gpa = find_space_from_ve820(vm, ELF_LOAD_PARAMS_SIZE,
				   MEM_4K, VIRT_RSDP_ADDR);

	if (load_params_gpa != INVALID_GPA) {
		/* We boot ELF Image from protected mode directly */
		init_vcpu_protect_mode_regs(vcpu, ELF_INITGDT_GPA(load_params_gpa));
		stac();
		...
		if (mb_hdr != NULL) {
			...
			if ((mb_hdr->flags & MULTIBOOT_HEADER_NEED_MEMINFO) != 0U) {
				mmap_length = prepare_multiboot_mmap(vm, ELF_MMAP_GPA(load_params_gpa));
				stac();
			}

			if (mmap_length != 0U) {
				mb_info.mi_flags |= MULTIBOOT_INFO_HAS_MMAP;
				mb_info.mi_mmap_addr = (uint32_t)ELF_MMAP_GPA(load_params_gpa);
				mb_info.mi_mmap_length = mmap_length;
			}
			ret = prepare_elf_cmdline(vm,  ELF_CMDLINE_GPA(load_params_gpa));
			stac();
			if (ret == 0) {
				mb_info.mi_flags |= MULTIBOOT_INFO_HAS_CMDLINE;
				mb_info.mi_cmdline = ELF_CMDLINE_GPA(load_params_gpa);
				ret = prepare_loader_name(vm, ELF_LDR_NAME_GPA(load_params_gpa));
				stac();
			}

			if (ret == 0) {
				mb_info.mi_flags |= MULTIBOOT_INFO_HAS_LOADER_NAME;
				mb_info.mi_loader_name = ELF_LDR_NAME_GPA(load_params_gpa);
				ret = copy_to_gpa(vm, (void *)&mb_info, ELF_MB_INFO_GPA(load_params_gpa),
			...
			if (ret == 0) {
				...
				vcpu_set_gpreg(vcpu, CPU_REG_RBX, ELF_MB_INFO_GPA(load_params_gpa));
